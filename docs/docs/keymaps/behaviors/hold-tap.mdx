...
Use the `zmk,behavior-hold-tap-bilateral` behavior to keep enforcing the positional
restriction until the hold-tap key is released. If a decided hold is active and a
...
bilateral_mt: bilateral_mod_tap {
    compatible = "zmk,behavior-hold-tap-bilateral";
    #binding-cells = <2>;
    flavor = "tap-preferred";
    tapping-term-ms = <280>;
    bindings = <&kp>, <&kp>;
...
On a non-split keyboard, this compatible behaves exactly like a regular
`zmk,behavior-hold-tap`. This lets shared keymaps use bilateral hold-taps without changing
the behavior of non-split boards.
