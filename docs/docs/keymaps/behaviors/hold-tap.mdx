...
After a hold-tap resolves as a hold, `linger-ms` keeps the hold behavior pressed for the
configured interval after the physical key is released. The default is `0`, which disables
lingering and preserves normal hold-tap behavior.
...
Pressing a different physical key during the interval releases the lingering hold before that
key is processed. Pressing the same physical key transfers the existing hold into a fresh
hold-tap decision without invoking the hold behavior twice. If that decision becomes a tap, the
tap behavior is pressed and released while the hold behavior remains active, and then the hold
behavior is released. If it becomes another hold, the existing hold remains active and a new
linger interval starts when the physical key is released again.

Lingering works with any behaviors accepted by the normal hold-tap `bindings` property, including
key presses, layer behaviors, and other one-parameter behaviors. The same-position transfer
requires the new invocation to resolve to a hold-tap with the same hold behavior and parameter.

On split keyboards, [`enforce-bilateral`](#bilateral-enforcement-enforce-bilateral) takes
precedence over `linger-ms`: a resolved hold is released immediately with the physical key and
cannot be inherited by a same-position press. On non-split keyboards, `enforce-bilateral` has no
effect, so a node containing both properties still uses its configured linger interval.
...
### Bilateral enforcement (`enforce-bilateral`)

By default, `hold-trigger-key-positions` only affects a hold-tap until its tapping term
expires. After that, a timer-decided hold remains active regardless of which key is pressed.

Add `enforce-bilateral;` to keep enforcing the positional restriction until the
hold-tap key is released on a split keyboard. If a decided hold is active and a
non-bilateral hold-tap key outside
`hold-trigger-key-positions` is pressed, the hold behavior is released and the hold-tap is
converted to its tap behavior before the new key is processed.

If another bilateral hold-tap is pressed outside the trigger positions, the first
hold-tap remains active until the second one is decided. A hold decision preserves the
same-hand modifier chord. A tap decision converts the earlier same-hand modifiers back to
their tap behaviors before emitting the new tap.

```dts
bilateral_mt: bilateral_mod_tap {
    compatible = "zmk,behavior-hold-tap";
    #binding-cells = <2>;
    flavor = "tap-preferred";
    tapping-term-ms = <280>;
...
This is useful for strict bilateral home-row modifiers because even a modifier held past its
tapping term will only modify keys in the configured positions, while same-hand modifier
chords remain available.

On a split keyboard, `enforce-bilateral` also disables `linger-ms`. Releasing the hold-tap
releases its hold behavior immediately, so a later press of the same physical position cannot
inherit it. On a non-split keyboard, `enforce-bilateral` has no effect, including no effect on
lingering. This lets shared keymaps use both properties while receiving strict bilateral behavior
on split boards and normal linger behavior on non-split boards.
