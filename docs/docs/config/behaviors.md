...
| `enforce-bilateral`           | bool     | On split keyboards, strictly enforces `hold-trigger-key-positions` after the hold decision and disables linger | false              |
| `linger-ms`                   | int      | Keeps a resolved hold behavior active for this long after release unless bilateral enforcement is active       | 0 (disabled)       |
